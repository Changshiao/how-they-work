...
def plot_success_rate(column, title, cmap="OrRd"):
    ...
    world.plot(column=column, ax=ax, legend=True, cmap=cmap,
               legend_kwds={"label": "Prosecution rate", "orientation": "vertical", "shrink": 0.6},
               missing_kwds={"color": "lightgrey", "label": "No Data"})
    ax.set_title(title, fontsize=20, fontweight="bold", color="#154360")
    ax.axis("off")
    plt.tight_layout()
    plt.show()

# 按照 Success Rate 绘制地图
plot_success_rate("prosecution_rate", "Cybercrimes Prosecution rate Distribution", cmap="GnBu")
